...
def horizontal_shift(img, nome, ratio=0.0):
    ...
    img = fill(img, h, w)
    cv2.imwrite("./Dataset/images/train/HS-"+nome, img)
...
def vertical_shift(img, nome, ratio=0.0):
    ...
    if ratio < 0:
        img = img[int(-1*to_shift):, :, :]
    img = fill(img, h, w)
    cv2.imwrite("./Dataset/images/train/VS-"+nome, img)
...
def brightness(img, low, high, nome):
    ...
    cv2.imwrite("./Dataset/images/train/BG-"+nome, img)

def horizontal_flip(img, flag, nome):
    if flag:
        img = cv2.flip(img, 1)
        cv2.imwrite("./Dataset/images/train/HF-"+nome, img)
    else:
        return img
...
def vertical_flip(img, flag, nome):
    if flag:
        img = cv2.flip(img, 0)
        cv2.imwrite("./Dataset/images/train/VF-"+nome, img)
    else:
        return img
...
def rotation(img, angle, nome):
    ...
    cv2.imwrite("./Dataset/images/train/RT-"+nome, img)

def noise(img, nome):
    img = random_noise(img)
    img = img_as_ubyte(img)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    cv2.imwrite('./Dataset/images/train/NS-'+nome, img)
    

#./BRA-Dataset/images/train/

#Lendo as imagens de treino
pasta = f'./Dataset/images/train'
    ...
    if len(arquivos) == 0:
        ...
        for arquivo in arquivos:
            if arquivo.endswith(".jpg"):
                img = cv2.imread('./Dataset/images/train/'+arquivo)
